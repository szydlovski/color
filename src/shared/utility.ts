...
export const compose = <T extends ArityOneFn[]>(...fns: T) => (p: FirstFnParameterType<T>): LastFnReturnType<T> => fns.reduceRight((acc: any, cur: any) => cur(acc), p);

export const composeConverters = <T extends ArityOneFn[]>(...fns: T) => (color: FirstFnParameterType<T>): LastFnReturnType<T> => fns.reduceRight((acc: any, cur: any) => cur(acc), color);
...
export function prettyJoin(values: any[], glue = ',', lastGlue = 'and'): string {
  if (values.length < 3) {
...
export function multiple([single, multiple]: [string, string], count: number): string {
